...
function toggle(source) {
    ...
    if (source.id === "selectAll") {
        ...
        selectAllCheckbox.checked = (checkedCount === checkboxes.length - 1);
    }
}
...
const drawing = () => cards.innerHTML = data.map((item) =>
    ...
    item.qnt > 0 ? (` <div id=${item.id} class="flex items-start flex-col md:flex-row md:items-center">
    <input onclick="toggle(this)" id="link-${item.id}" type="checkbox" value="" class="w-4 h-4 mb-5 md:mb-0 me-3 md:me-5 text-[#F49B57] bg-gray-200 border-gray-300 rounded focus:ring-[#F49B57] focus:ring-2">




   <label class="flex-1 w-full" for="link-${item.id}">
   <span 
